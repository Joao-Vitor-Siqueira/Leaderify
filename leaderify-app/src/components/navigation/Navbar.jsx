...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      <div className = {`flex justify-between w-full items-center pr-2 ${isOpen ? "h-1/4" : "h-full"} min-h-1/4 ${hambContainerDesktop}`}>
        <IconButton onClick={toggle} iconName={"hamburguer"} sizing={`h-full lg:h-16 w-11 lg:w-full`}/>
        <ProfileButton className={"w-10 aspect-square lg:hidden"}/>
        <IconButton onClick={() => {}} tooltip={"Settings"} iconName={"settings"} sizing={`hidden h-16 w-full lg:inline-block fixed bottom-0 border-t border-mauve-300`} fill={true} iconColor={"text-mauve-600"}/>
      </div>

      {/* Open navbar */} 

      <div className= {`${isOpen ? "" : "hidden"} lg:hidden`}><Divider/></div> 

      <ul className= {`${isOpen ? "grow flex flex-col overflow-y-auto" : "hidden"} lg:order-1`}>
        <div className={`text-lg text-center p-4 bg-mauve-400 hidden lg:block h-16`}>Leaderify</div>
        ...
        <div className="hidden lg:flex items-center justify-between max-[1600px]:justify-start max-[1350px]:gap-1 gap-2 absolute bottom-0 pl-2 w-full max-w-[85%] h-16 border-t border-mauve-300">
          <div className="flex gap-3 items-center">
            ...
            <p className="text-lg max-[1350px]:text-base max-[1250px]:hidden">Username</p>
          </div>
          <IconButton onClick={() => {}} tooltip={"Friends"} iconName={"friends"} sizing={`hidden h-16 w-11 lg:inline-block`} fill={true} iconColor={"text-mauve-600"}/>
        </div>
      </ul>
